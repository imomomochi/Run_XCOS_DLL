...
int  TestBlock(scicos_block *block, int flag)
{
  ...
  memcpy(*(TestBlock_block_outtbptr+1),*(u+0),1*sizeof(double));

  if (flag != 4 && flag != 6 && flag != 5){
    reentryflag=(int*) ((scicos_block *)(*block->work)+10);
    if (*reentryflag==0){
      *reentryflag=1;
  ...
  if (flag == 1) { /* Output computation */
    ...
    if ((*block->work=scicos_malloc(sizeof(scicos_block)*10+sizeof(int)))== NULL) return 0;
    reentryflag=(int*) ((scicos_block *)(*block->work)+10);
    *reentryflag=0;
    block_TestBlock=(scicos_block*) *block->work;
  ...
  memcpy(*(y+0),*(TestBlock_block_outtbptr+0),1*sizeof(double));
  memcpy(*(y+1),*(TestBlock_block_outtbptr+3),1*sizeof(double));
  return 0;
